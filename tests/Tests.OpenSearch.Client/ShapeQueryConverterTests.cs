...
{
	/// <summary>
	/// Behavioural tests for the settings-aware <see cref="ShapeQueryConverter"/> (folds the legacy
	/// <c>CompositeFormatter&lt;IShapeQuery, ShapeQueryFormatter, ShapeQueryFieldNameFormatter&gt;</c> read/write
	...
	/// </summary>
	public class ShapeQueryConverterTests
	{
		...
		private static IShapeQuery Deserialize(string json) =>
			JsonSerializer.Deserialize<IShapeQuery>(json, Options());

		[U] public void Serialize_ShapeBranch_WrapsUnderFieldName()
		{
			IShapeQuery query = new ShapeQuery
			{
				Field = "location",
				Name = "n",
				Boost = 1.1,
		...
		[U] public void Deserialize_ShapeBranch_ReadsFieldAndShape()
		{
			...
			query.Relation.Should().Be(ShapeRelation.Intersects);
		...
		[U] public void Deserialize_IndexedShapeBranch_ReadsFieldLookup()
		{
			...
			query.Relation.Should().Be(ShapeRelation.Within);
		...
		{
			IShapeQuery query = new ShapeQuery { Shape = new PointGeoShape(new GeoCoordinate(20, 10)) };
			JsonSerializer.Serialize(query, Options()).Should().Be("null");
		}

		[U] public void Serialize_NullValue_WritesNull() =>
			JsonSerializer.Serialize<IShapeQuery>(null, Options()).Should().Be("null");

		[U] public void RoundTrip_ShapeBranch()
		{
			IShapeQuery original = new ShapeQuery
			{
				Field = "location",
				Name = "n",
				Boost = 2.0,
