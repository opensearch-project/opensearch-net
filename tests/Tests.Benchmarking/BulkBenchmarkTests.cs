...
namespace Tests.Benchmarking
{
	...
	public class BulkBenchmarkTests
	{
		...
		private static readonly IOpenSearchClient ClientNoRecyclableMemory =
			new OpenSearchClient(new ConnectionSettings(new InMemoryConnection(Response, 200, null, null))
				.DefaultIndex("index")
				.EnableHttpCompression(false)
			);

		private static readonly Nest7.IOpenSearchClient ClientV7 =
			new Nest7.OpenSearchClient(new Nest7.ConnectionSettings(
					new OpenSearch.Net7.InMemoryConnection(Response, 200, null, null))
				.DefaultIndex("index")
				.EnableHttpCompression(false)
			);
