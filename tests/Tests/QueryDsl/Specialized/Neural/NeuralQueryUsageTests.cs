...
public class NeuralQueryCluster : ClientTestClusterBase
{
    ...
    protected override void SeedNode()
    {
        ...
        var deployModelResp = Client.Http.Post<DynamicResponse>($"/_plugins/_ml/models/{ModelId}/{(renamedToRegisterDeploy ? "_deploy" : "_load")}");
        ...
        var updateIndexSettingsResp = Client.Indices.UpdateSettings(".plugins-ml-*", u => u.ExpandWildcards(ExpandWildcards.Hidden).IndexSettings(s => s.NumberOfReplicas(0)));
