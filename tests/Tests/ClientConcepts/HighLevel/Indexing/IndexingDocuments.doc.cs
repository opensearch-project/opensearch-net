	...
	public class Indexing : DocumentationTestBase
	{
		...
		*/
		public async Task SingleDocument()
		{
			...
			var indexResponseAsync = await client.IndexDocumentAsync(person); //<2> asynchronous method that returns a `Task<IndexResponse>` that can be awaited
		}
		...
		*/
		public void SingleDocumentWithParameters()
		{
			...
			var indexResponse1 = client.Index(person, i => i.Index("people")); //<1> fluent syntax

			var indexResponse2 = client.Index(new IndexRequest<Person>(person, "people")); //<2> object initializer syntax
		}
