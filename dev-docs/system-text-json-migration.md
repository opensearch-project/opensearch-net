...
### Known flaky / environmental integration tests (not regressions)

After the response fixes above, the integration suite is green, but a few tests are
...
The one unit-level flaky (`FieldInference.PrecedenceIsAsExpected`) was root-caused and fixed
(see section 55); it is included here only for completeness.
