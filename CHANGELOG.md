...
- Added an interface-aware `DataContractResolver` and a `[StringEnum]`-aware `StringEnumConverterFactory` for `System.Text.Json`, plus a reusable `PolymorphicInterfaceConverter<T>`, wiring the analysis tokenizers, char filters and token filters namespaces (and the `StopWords` union type) through the new serializer with byte-for-byte parity against the existing client ([#388](https://github.com/opensearch-project/opensearch-net/issues/388)).
