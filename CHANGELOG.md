...
- Generate high-level `ml` and `search_pipeline` namespace clients (typed requests, responses, descriptors, shared models, and wrapper-key discriminated unions) from the OpenAPI spec via plugin-based model code generation (`--include-high-level`). Includes `GlobalMappedTypes` for rich C# types (Id, IndexName, Indices, IIndexSettings), `PropertyTypeOverrides` for wrapper types (IProperties, IAliases), cross-language wire-compatibility tests, and an AOS sample with SigV4 signing ([#1017](https://github.com/opensearch-project/opensearch-net/pull/1017))
