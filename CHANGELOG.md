...
- Added `System.Text.Json` migration infrastructure — an interface-aware `DataContractResolver` (honoring `[DataMember]`/`ShouldSerialize`/non-public setters/constructor-less types), a `[StringEnum]`-aware `StringEnumConverterFactory`, an `ObjectConverter` for dynamic payloads, `DoubleFormatConverter`/`SingleFormatConverter` for Utf8Json-compatible number formatting, a reusable `PolymorphicInterfaceConverter<T>`, settings-threaded `Field`/`PropertyName` converters with field-name inference, and a consolidated options factory — and wired the entire analysis area (tokenizers, char filters, token filters, analyzers, normalizers, `StopWords`), the `IScript` hierarchy, the `QueryContainer` dispatch, and the field-name-keyed queries (term/prefix/wildcard/regexp/match/match_phrase/…) through the new serializer with byte-for-byte parity against the existing client ([#388](https://github.com/opensearch-project/opensearch-net/issues/388)).
