...
- Added `System.Text.Json` migration infrastructure — an interface-aware `DataContractResolver` (honoring `[DataMember]`/`ShouldSerialize`/non-public setters/constructor-less types), a `[StringEnum]`-aware `StringEnumConverterFactory`, an `ObjectConverter` for dynamic payloads, `DoubleFormatConverter`/`SingleFormatConverter` for Utf8Json-compatible number formatting, a reusable `PolymorphicInterfaceConverter<T>`, settings-threaded `Field`/`PropertyName` converters with field-name inference, and a consolidated options factory — and wired the entire analysis area (tokenizers, char filters, token filters, analyzers, normalizers, `StopWords`), the `IScript` hierarchy, the `QueryContainer` dispatch, the bulk of the query DSL (field-name-keyed, range, compound, joining, span, terms/ids, full-text, geo-distance and rank-feature queries; with `Id`/`Fields`/`RelationName`/`DateMath`/`Distance`/`GeoLocation` converters, `ShouldSerialize`/explicit-interface/`[InterfaceDataContract]`/`[ReadAs]` resolver support), the request-side aggregations (metric, bucket, nested sub-aggregations via the `aggs` dictionary; incl. the bespoke `filter`, `percentiles`/`percentile_ranks` and `composite` source shapes, `terms` include/exclude and `terms`/`histogram` order, `buckets_path`, and calendar/fixed interval `Union`/`Time`/`DateMathTime` values), the `geo_bounding_box` and `distance_feature` queries (with a `GeoCoordinate` array converter and a generic `Union` converter), a suite of cross-cutting value-type converters (`IndexName`, `Indices`, `Routing`, `TaskId`, `Sort`, `SourceFilter`, `Slices`, `ReindexRouting`, `JoinField`, `Similarity`, `AutoExpandReplicas`), and a per-property converter mechanism (via `DataContractResolver.PropertyConverterOverrides`) that honors member-level `[JsonFormatter]` attributes for the primitive string-or-number/bool formatters (`NullableStringBoolean`/`Int`/`Long`/`Double`, `StringInt`/`Long`, `IntString`) which the server may send as JSON strings, converters for `Fuzziness`, `Attachment`, `IAliasAction` (add/remove/remove_index) and `indices_boost`, an open-generic extension of the per-property mechanism with `single-or-enumerable` / `serialize-as-single` converters (lenient single-value reads), the `neural` query and the `MultiTermQueryRewrite` (`rewrite`) option, the `fuzzy` query (string/numeric/date) and the `more_like_this` `Like` union, the verbatim dictionary-key family (analyzers/token-filters/char-filters/tokenizers/normalizers/similarities/relations/runtime-fields/aliases/etc. — keys written as inferred, incl. `Field`/`IndexName`/`RelationName` keys) via type-level `[JsonFormatter]` resolution, the `intervals` query, and the `IGeoShape` GeoJSON geometry hierarchy (point/multipoint/linestring/multilinestring/polygon/multipolygon/envelope/circle/geometrycollection, incl. WKT) and the `geo_shape` and `shape` queries (inline shape or indexed shape + relation), and the mapping properties (type-discriminated `IProperty` union + the named `properties` dictionary, incl. multi-fields and nested/object sub-properties), a first slice of the aggregation response reader (value/stats/single-bucket/multi-bucket with recursive sub-aggregations), response-side value readers (`TotalHits`, `TrackTotalHits`, `KeyedProcessorStats`, `CatFielddataRecord`), the dictionary/dynamic response family (a converter factory covering `GetPipeline`/`GetIndex`/`GetMapping`/`GetFieldMapping`/`GetAlias`/`GetIndexSettings`/`GetIndexTemplate`/`RemoteInfo`/cluster-state and similar `DictionaryResponseBase` types), the polymorphic `bulk` response items and the `suggest` dictionary (with `[ReadAs]` and per-property support for open generics), the `GetRepository` response (repository-type dispatch), the ingest pipeline `IProcessor` polymorphic dispatch (~35 processor types), mapping `dynamic_templates`, index settings (the dotted-key flatten/unflatten `IIndexSettings`/`IDynamicIndexSettings`), join `Children`, mapping `dynamic`, the `cluster reroute` commands, and `LazyDocument`/`FieldValues` (deferred deserialization of hit fields/source), the typed document `_source` reader (routing `[JsonFormatter(typeof(SourceFormatter<>))]` members — hit `_source`, update `doc`/`upsert`, term-vector/percolate documents — through the configured source serializer), and the `multi_search`/`multi_get` response builders (splitting the response envelope into per-operation segments and deserializing each into the originating operation's `SearchResponse<T>`/`MultiGetHit<T>`), and the source serializer's document field-name inference (a resolver that camel-cases document properties by default and honors configured property mappings, `DefaultMappingFor`, and `[PropertyName]`/`[DataMember]`/`[Ignore]` attributes, matching the previous source serialization) through the new serializer with byte-for-byte parity against the existing client ([#388](https://github.com/opensearch-project/opensearch-net/issues/388)).
