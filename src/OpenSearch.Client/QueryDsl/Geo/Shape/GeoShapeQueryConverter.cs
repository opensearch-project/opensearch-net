...
{
	/// <summary>
	/// A <see cref="System.Text.Json"/> converter for <see cref="IGeoShapeQuery"/>, replacing the
	/// vendored Utf8Json <c>CompositeFormatter&lt;…, GeoShapeQueryFormatter, GeoShapeQueryFieldNameFormatter&gt;</c>
	/// as part of #388. Field-name-keyed: <c>_name</c>/<c>boost</c>/<c>ignore_unmapped</c> are written as
	/// siblings of the inferred field key, whose value holds the <c>shape</c> (an
	/// <see cref="IGeoShape"/>) or <c>indexed_shape</c> plus the <c>relation</c>.
	/// </summary>
	internal sealed class GeoShapeQueryConverter : JsonConverter<IGeoShapeQuery>
	{
		private readonly IConnectionSettingsValues _settings;

		public GeoShapeQueryConverter(IConnectionSettingsValues settings) =>
						...
						foreach (var shapeMember in member.Value.EnumerateObject())
						{
							switch (shapeMember.Name)
							{
								...
								case "indexed_shape":
									...
									break;
								case "relation":
									query.Relation = shapeMember.Value.Deserialize<GeoShapeRelation>(options);
									break;
							}
						}
						break;
