...
namespace OpenSearch.Client
{
	...
	/// <c>CompositeFormatter&lt;IShapeQuery, ShapeQueryFormatter, ShapeQueryFieldNameFormatter&gt;</c>.
	///
	/// The composite dispatched read and write to two different sub-formatters; this converter folds both paths in:
	/// <list type="bullet">
	/// <item><description><b>Write</b> (legacy <c>ShapeQueryFieldNameFormatter</c>): emits the field-name wrapper
	...
	/// <item><description><b>Read</b> (legacy <c>ShapeQueryFormatter</c>): reads the same shape; the property key
	...
	/// </list>
	/// The <see cref="IGeoShape"/> body, <see cref="IFieldLookup"/>, and <see cref="ShapeRelation"/> are delegated to
	/// <see cref="JsonSerializer"/> so the registered converters apply.
	/// </summary>
	internal class ShapeQueryConverter : SettingsAwareConverter<IShapeQuery>
	{
		public ShapeQueryConverter(IConnectionSettingsValues settings) : base(settings) { }
