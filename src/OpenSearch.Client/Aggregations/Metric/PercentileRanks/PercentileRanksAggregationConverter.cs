...
{
	/// <summary>
	/// A <see cref="System.Text.Json"/> converter for <see cref="IPercentileRanksAggregation"/>,
	/// replacing the vendored Utf8Json <c>PercentileRanksAggregationFormatter</c> as part of #388. The
	/// <c>values</c> array and the <c>tdigest</c>/<c>hdr</c> method are declared on the interface
	/// without <c>[DataMember]</c>, so this converter writes the full shape explicitly.
	/// </summary>
	internal sealed class PercentileRanksAggregationConverter : JsonConverter<IPercentileRanksAggregation>
	{
		public override void Write(Utf8JsonWriter writer, IPercentileRanksAggregation value, JsonSerializerOptions options)
		{
			...
			if (value.Values != null)
			{
				...
				JsonSerializer.Serialize<IEnumerable<double>>(writer, value.Values, options);
			}
