...
{
	/// <summary>
	/// System.Text.Json replacement for the legacy Utf8Json <c>MultiSearchFormatter</c>.
	///
	/// A multi-search request body is <em>newline-delimited JSON</em> (ndjson): for each search operation a header
	/// object (index/search_type/preference/routing/ignore_unavailable) is written followed by a raw <c>'\n'</c>,
	/// then the search body followed by another raw <c>'\n'</c>.
	///
	/// A <see cref="Utf8JsonWriter"/> forbids emitting more than one JSON value at the document root and cannot emit
	/// a bare newline between values, so the whole payload is built into a buffer (using nested writers that inherit
	/// the serializer's encoder) and emitted verbatim with a single
	...
	/// </summary>
	internal class MultiSearchConverter : SettingsAwareConverter<IMultiSearchRequest>
	{
		private const byte Newline = (byte)'\n';

		public MultiSearchConverter(IConnectionSettingsValues settings) : base(settings) { }

		// The legacy Deserialize did NOT throw; it delegated to a dynamic camelCase resolver. The closest faithful
		...
		public override IMultiSearchRequest Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options) =>
			JsonSerializer.Deserialize<MultiSearchRequest>(ref reader, options);
