...
namespace Osc
{
	...
	public interface IMultiMatchQuery : IQuery
	{
		...
		/// (below the cutoff) terms in the case of <see cref="Osc.Operator.Or" />,
		/// or all of the low frequency terms in the case of an <see cref="Osc.Operator.And" /> match.
		/// </summary>
		...
		/// The default operator is <see cref="Osc.Operator.Or" />
		/// </summary>
		/// <remarks>
		...
		/// to match no documents at all. In order to change that, <see cref="Osc.ZeroTermsQuery" /> can be used,
		/// which accepts <see cref="Osc.ZeroTermsQuery.None" /> (default) and <see cref="Osc.ZeroTermsQuery.All" />
