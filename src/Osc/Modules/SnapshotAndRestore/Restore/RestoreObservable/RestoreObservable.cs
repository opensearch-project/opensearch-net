...
namespace Osc
{
	public class RestoreObservable : IDisposable, IObservable<RecoveryStatusResponse>
	{
		...
		public RestoreObservable(IOpenSearchClient elasticClient, IRestoreRequest restoreRequest)
		{
		...
		public RestoreObservable(IOpenSearchClient elasticClient, IRestoreRequest restoreRequest, TimeSpan interval)
		...
		public IDisposable Subscribe(IObserver<RecoveryStatusResponse> observer)
		{
			...
			try
			{
				...
				if (!restoreResponse.IsValid)
					throw new OpenSearchClientException(PipelineFailure.BadResponse, "Failed to restore snapshot.", restoreResponse.ApiCall);
	...
	public class RestoreStatusHumbleObject
	{
		...
		public RestoreStatusHumbleObject(IOpenSearchClient elasticClient, IRestoreRequest restoreRequest)
		{
			elasticClient.ThrowIfNull(nameof(elasticClient));
			restoreRequest.ThrowIfNull(nameof(restoreRequest));
		...
		public void CheckStatus()
		{
			try
			{
				...
				if (!recoveryStatus.IsValid)
					throw new OpenSearchClientException(PipelineFailure.BadResponse, "Failed getting recovery status.", recoveryStatus.ApiCall);
