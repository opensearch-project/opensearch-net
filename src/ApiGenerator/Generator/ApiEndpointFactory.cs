...
public static class ApiEndpointFactory
{
	...
	private static void PatchOfficialSpec(JObject original, string jsonFile)
	{
		...
		original.Merge(patchedJson, new JsonMergeSettings { MergeArrayHandling = MergeArrayHandling.Union });
		...
		originalParams?.Merge(paramsOverride, new JsonMergeSettings { MergeArrayHandling = MergeArrayHandling.Union });
	...
	private static void TransformNewSpecStructureToOld(JObject original)
	{
		...
		foreach (var path in spec["url"]["paths"].Cast<JObject>())
		{
			...
			if (path.ContainsKey("parts"))
			{
				...
				{
					parts.Merge(path["parts"].Value<JObject>(), new JsonMergeSettings { MergeArrayHandling = MergeArrayHandling.Union });
				}
			}
		...
		var newUrl = new JObject { ["paths"] = new JArray(paths.Cast<object>().ToArray()) };
